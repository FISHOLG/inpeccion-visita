...
const CardVisita = ({vehiculo,seleccionarVehiculo}:Props) => {
  ...
  return (
      <Pressable
          ...
          className={
              `flex-1 flex-row gap-x-8 mb-5 p-5  rounded-2xl active:opacity-60 ${vehiculo.tipoInspeccion==='I'?'bg-light-success dark:bg-dark-success':'bg-light-danger dark:bg-dark-danger'}`
          }
      >
         ...
         <View className={'flex-1 justify-between gap-8'}>
             ...
             <ThemedText type={'h2'} className={'uppercase text-right text-light-textMain dark:text-dark-textMain'}> {vehiculo.tipoInspeccion==='I'?'Ingreso':'Salida'}</ThemedText>
         </View>
      </Pressable>
  );
};
